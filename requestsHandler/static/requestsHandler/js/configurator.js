...
function loadConfig(hash) {
  $.get("/getConfig", {"hash":hash}, function( data ) {
      ...
      if (!config["valid_amo"]) alert("Неверные данные для входа в amoCRM!");
      if (hash == "#accesses") {
        var schema = {
      ...
      for (var field in config["site_fields_conformity"]["contact_data"]) {
        contact_fields.push({
          "amoCRM" : field,
          "site" : config["site_fields_conformity"]["contact_data"][field]
        })
      }
      var contact_fields_to_check_dups = new Array();
      for (var field in config["fields_to_check_dups"]["contacts"]) {
        contact_fields_to_check_dups.push({
          "field" : config["fields_to_check_dups"]["contacts"][field],
        })
      }
      
      var company_fields = new Array();
      for (var field in config["site_fields_conformity"]["company_data"]) {
        company_fields.push({
          "amoCRM" : field,
          "site" : config["site_fields_conformity"]["company_data"][field]
        })
      }
      var company_fields_to_check_dups = new Array();
      for (var field in config["fields_to_check_dups"]["companies"]) {
        company_fields_to_check_dups.push({
          "field" : config["fields_to_check_dups"]["companies"][field],
        })
      }
      
      var lead_fields = new Array();
      for (var field in config["site_fields_conformity"]["lead_data"]) {
        lead_fields.push({
          "amoCRM" : field,
          "site" : config["site_fields_conformity"]["lead_data"][field]
        })
      }
      ...
      if (hash == "#accesses") {
        ...
        editor.setValue({
          ...
          "generate_tasks_for_rec" : config["site_fields_conformity"]["generate_tasks_for_rec"],
          "rec_lead_task_text" : config["rec_lead_task_text"],
          "time_to_complete_rec_task" : config["time_to_complete_rec_task"]/60,
          "tag_for_rec" : config["site_fields_conformity"]["tag_for_rec"],
      ...
      $(".send").click(function(){
        ...
        var jqxhr = $.post("/setConfig", JSON.stringify(editor.getValue()), function() {
          location.reload();
        }, "json");
      });
